...
其中线宽和字号均使用毫米。输出目录通过 `config.yaml` 的 `output_dir` 设置，例如 `output_dir: img/output`；相对路径以配置文件所在目录为基准，未配置时使用该目录下的 `output/`。显式传入 `--output` 会覆盖配置，其相对路径以当前工作目录为基准。上文输出结构以 `output/` 为例，日志、汇总文件及续跑检查均使用实际输出目录。图片区通过 `--images` 设置。
